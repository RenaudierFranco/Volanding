      ...
      <Card.Deck className="text-center">
        {items.map((item) => (
          <Card key={item.id} shadow p="3" mb="3" bg="light" rounded>
            <Card.Body>
              <div><Card.Title mb="3"><strong>{item.item.departure} - {item.item.arrival}</strong></Card.Title></div>
              ...
              <div><Button danger outline variant="danger" mt="3" ml="3" onClick={() => deleteFlight(item.id)}>Dar de Baja</Button></div>
            </Card.Body>
          </Card>
        ))}
      </Card.Deck>
